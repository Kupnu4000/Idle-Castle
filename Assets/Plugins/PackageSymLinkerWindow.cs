...
public class PackageSymLinkerWindow : EditorWindow, IHasCustomMenu
{
	...
	{
		PackageSymLinkerWindow window = GetWindow<PackageSymLinkerWindow>();
		window.Show();
	}

	[SerializeField] private List<SymlinkedDirInfo> _symLinks = new List<SymlinkedDirInfo>();
	[SerializeField] private Vector2                _scroll;
	[SerializeField] private string[]               _recentPackagesPaths = Array.Empty<string>();
	[SerializeField] private Package[]              _recentPackageInfo   = Array.Empty<Package>();
	...
	private void OnLinkedPackagesGUI ()
	{
		...
		foreach (SymlinkedDirInfo symLink in _symLinks)
		{
	...
	{
		if (_recentPackagesPaths.Length == 0)
		{
			return;
		}
		...
		for (var index = 0; index < _recentPackagesPaths.Length; index++)
		{
			var     packagePath = _recentPackagesPaths[index];
			Package package     = _recentPackageInfo[index];
	...
	private void ReloadLinkedPackages ()
	{
		...
		_symLinks = dirPaths
		            ...
		            .Select(static path =>
		            {
			            ...
			            Package packageInfo        = JsonUtility.FromJson<Package>(packageJsonString);
		...
		titleContent = _symLinks.Count > 0
			? new GUIContent($"Package Symlinker ({_symLinks.Count})")
			: new GUIContent($"Package Symlinker");
	}
	...
	private static int TryExecuteCmd (string command, out string output, out string error)
	{
		#if UNITY_EDITOR_WIN
		var cmd  = "cmd.exe";
		var args = $"/c {command}";
      ...
      var args = $"-c \"{command}\"";
		...
		launchProcess.OutputDataReceived += (_, e) => outputBuilder.AppendLine(e.Data ?? "");
		launchProcess.ErrorDataReceived  += (_, e) => errorBuilder.AppendLine(e.Data ?? "");
	...
	{
		_recentPackagesPaths = EditorPrefs.GetString(RecentPackagesPrefsKey, "")
		                                  .Split(RecentPackagesSeparator, StringSplitOptions.RemoveEmptyEntries);

		_recentPackageInfo = new Package[_recentPackagesPaths.Length];

		for (var i = 0; i < _recentPackageInfo.Length; i++)
		{
			var packageJsonPath = Path.Combine(_recentPackagesPaths[i], "package.json");
			...
			_recentPackageInfo[i] = packageInfo;
		}
	}
	...
	private void AddPackageToRecent (string path)
	{
		...
		var list = _recentPackagesPaths.ToList();
		list.Remove(path);
		list.Add(path);
		_recentPackagesPaths = list.ToArray();
		EditorPrefs.SetString(RecentPackagesPrefsKey, string.Join(RecentPackagesSeparator, _recentPackagesPaths));
	...
	{
		foreach (SymlinkedDirInfo dir in _symLinks)
		{
